...
prov_path="${PROV_PATH:-$HOME/go/1.23.7/src/github.com/provenance-io/provenance/build/run/mantrachaind}"
...
feebucket=$(mantrachaind keys show feebucket -a --keyring-backend "$KEYRING" --home "$prov_path" --testnet)
echo "Using feebucket: $feebucket"

# --- query on-chain account to avoid sequence mismatch ---
acct_json=$(mantrachaind q auth account "$feebucket" --home "$prov_path" --testnet --node "$NODE" --output json) || {
